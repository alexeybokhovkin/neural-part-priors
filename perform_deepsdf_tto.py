...
def main(args):
    ...
    train_dataset = VoxelisedImplicitScanNetDataset(datadir=config.datadir_trees,
                                                    ...
                                                    object_list=config.dataset_list,
                                                    ...
                                                    mlcvnet_noise_path=config.mlcvnet_noise_path, 
    ...
    with open(os.path.join(config.datadir_trees, config.dataset, config.dataset_list), 'r') as fin:
        test_samples = fin.readlines()
        test_samples = [x[:-1] for x in test_samples]

    device = torch.device('cuda:0')
    model = GNNPartnetLightning(config, cat_name=args.cat_name, mode='tto')
