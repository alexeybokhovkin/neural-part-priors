...
def main(args):
    ...
    sa_samples = ['38012_scene0609_02_4', '42928_scene0609_02_13', '23140_scene0609_02_0',
                 '43297_scene0196_00_0', '42021_scene0196_00_13', '23487_scene0196_00_5',
                 '12795_scene0435_01_3', '36398_scene0549_00_6', '24001_scene0549_00_8',
                 '28514_scene0549_00_10', '28514_scene0549_00_11', '3334_scene0621_00_26',
                 '42021_scene0621_00_27', '40905_scene0257_00_8', '43310_scene0257_00_3',
                 '40905_scene0257_00_6']

    STORED_RESULTS_DIR = '/cluster/valinor/abokhovkin/scannet-relationships/test_output_full_eccv/meshes_scannet'
    mesh_dir = f'deepsdf_parts_fullshape_mlcvnet_finetune_predlat_predpoints_{args.cat_name}_0.25_0'
    # STORED_RESULTS_DIR = '/cluster/valinor/abokhovkin/scannet-relationships-v2/test_output_full_cvpr/meshes_scannet'
    # mesh_dir = f'deepsdf_parts_fullshape_mlcvnet_finetune_predlat_predpoints_{args.cat_name}'

    with open(os.path.join(STORED_RESULTS_DIR, mesh_dir, 'scene_aware_cd.json'), 'r') as fin:
        scene_aware_map = json.load(fin)

    scene_ids = sorted(list(scene_aware_map.keys()))
    with open(os.path.join(datadir, dataset, dataset_list), 'r') as fin:
        test_samples_list = fin.readlines()
        test_samples_list = [x[:-1] for x in test_samples_list]
    test_samples = {x: i for i, x in enumerate(test_samples_list)}

    for k, scene_id in enumerate(scene_ids[:]):
        print('scene', k, scene_id)
        for source_sample_name in scene_aware_map[scene_id]:
            # if source_sample_name not in sa_samples:
            #     continue
            source_sample_id = test_samples[source_sample_name]
            target_sample_names = scene_aware_map[scene_id][source_sample_name]
            target_sample_names = list(set(target_sample_names))
            for num_shapes in [3]: # 3, 6 (min)
                for k_near in [30]: # 30
                    for sa_mode in ['min']:

                        SAVE_BASEDIR = f'/cluster/valinor/abokhovkin/scannet-relationships-v2/test_output_full_cvpr_sa_old'
                        exp_full_name = exp + f'_predlat_predpoints_{args.cat_name}'
                        ...
                        for i in [source_sample_id]:
                            ...
                            TTO_DIR = os.path.join(SAVE_BASEDIR, 'meshes_scannet', exp_full_name, source_sample_name)
                            if os.path.exists(TTO_DIR + '_tto'):
                                continue

                            output = model.tto_two_stage(batch, index=source_sample_name, only_align=args.only_align, bck_thr=0.25,
                                                         ...
                                                         lr_dec_part=args.lr_dec_part, sa_mode=sa_mode,
                                                         store_dir=os.path.join(SAVE_BASEDIR, 'meshes_scannet', exp_full_name, source_sample_name))

                            t3 = time.time()

                            SAVE_DIR = os.path.join(SAVE_BASEDIR, 'meshes_scannet', exp_full_name, source_sample_name)
                            os.makedirs(SAVE_DIR, exist_ok=True)
                            ...
                            if args.only_align:
                                ...
                                for j, part_name in enumerate(pred_sdfs):
                                    try:
                                        ...
                                        print('Bad part before TTO:', source_sample_name)
                                        continue
                                ...
                                try:
                                    ...
                                    print('Bad full shape before TTO:', source_sample_name)

                                SAVE_DIR = os.path.join(SAVE_BASEDIR, 'meshes_scannet', exp_full_name, source_sample_name + '_tto')
                                os.makedirs(SAVE_DIR, exist_ok=True)
                                # for x in os.listdir(SAVE_DIR):
                                #     if not x.endswith('.pth'):
                                #         os.remove(os.path.join(SAVE_DIR, x))

                                print('Save after TTO')
                                    ...
                                    try:
                                        for k in range(len(pred_sdfs[part_name])):
                                            try:
                                                ...
                                                print('Bad part after TTO:', source_sample_name)
                                                continue
                                ...
                                for j in range(len(shape_sdf)):
                                    for k in range(len(shape_sdf[j])):
                                        try:
                                            ...
                                            print('Bad full shape after TTO:', source_sample_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scene_id', required=False, default='0', type=str)
