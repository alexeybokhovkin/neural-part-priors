# The code is taken from GitHub repo: https://github.com/daerduoCarey/structurenet
...
class Tree(object):
    ...
    @staticmethod
    def load_category_info(root_dir):
        with open(os.path.join(root_dir, 'full_traverse.txt'), 'r') as fin:
    ...
    class Node(object):
        ...
        def __init__(self, part_id=0, complete_part_id=None, is_leaf=False, label=None, children=None, edges=None, full_label=None,
                     geo=None, geo_feat=None):
            ...
            self.complete_part_id = complete_part_id
            self.geo = geo  # 1 x 64 x 64 x 64 voxel mask
        ...
        def graph(self, leafs_only=False):
            ...
            return part_geos, edges, part_ids, part_sems
        ...
        def edge_tensors(self, edge_types, device, type_onehot=True):
            ...
            edge_type = torch.tensor([edge_types.index(edge['type']) for edge in self.edges], device=device,
                                     dtype=torch.long)
            if type_onehot:
                edge_type = one_hot(inp=edge_type, label_count=len(edge_types)).transpose(0, 1).view(1, num_edges, len(
                    edge_types)).to(dtype=torch.float32)
            else:
    ...
    def save_object(obj, fn):
        ...
        while len(stack) > 0:
            ...
            for child in node.children:
                ...
                stack.append(
                    StackElement(node=child, parent_json=node_json, parent_child_idx=len(node_json['children']) - 1))
            ...
            for edge in node.edges:
                ...
                if 'params' in edge:
                    node_json['edges'][-1]['params'] = node_json['edges'][-1]['params'].cpu().numpy().reshape(
                        -1).tolist()
