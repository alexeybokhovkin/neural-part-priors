...
from src.utils.losses import NTXentLoss, NTXentLossChildren, NTXentLossChildrenFmaps
from src.utils.losses import NTXentLossClean, NTXentLossChildrenClean, NTXentLossChildrenFmapsClean
from src.utils.losses import NTXentLossChildrenPartwise, NTXentLossChildrenFmapsPartwise
...
class Unet3DGNNPartnetLightning(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.root_contrastive_loss = NTXentLossClean(torch.device("cuda:0"), config['batch_size'], 0.5, True)
        self.children_contrastive_loss = NTXentLossChildrenClean(torch.device("cuda:0"), config['batch_size'], 0.5)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # batch[11] = [x[None, ...] for x in batch[11]]
        # scannet_geos_neg = torch.cat(batch[11]).unsqueeze(dim=1)
        # shape_mask_neg = torch.cat(batch[12]).unsqueeze(dim=1)
        # gt_trees_neg = batch[13]
