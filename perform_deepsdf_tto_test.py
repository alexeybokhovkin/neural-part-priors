...
def main(args):
    ...
    dataset = 'all_chair_mlcvnet_valsplits'
    ...
    sdf_data_source = '/cluster/daidalos/abokhovkin/DeepSDF/ShapeNetV2_dim256_parts_mlcvnet/test/SdfSamples/ShapeNetV2'
    ...
    dataset_list = f'val_{num_split}.txt'
    print('Object list', dataset_list)
    ...
    config['dataset'] = 'all_chair_mlcvnet_valsplits'
    ...
    for i in range(len(test_samples)):
        ...
        SAVE_DIR = os.path.join(SAVE_BASEDIR, 'meshes_scannet', exp, test_samples[i] + '-tto')
        os.makedirs(SAVE_DIR, exist_ok=True)
        for x in os.listdir(SAVE_DIR):
            if not x.endswith('.pth'):
                os.remove(os.path.join(SAVE_DIR, x))
